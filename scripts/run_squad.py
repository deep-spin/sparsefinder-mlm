...
from transformers import (
    ...
    squad_convert_examples_to_features, RobertaTokenizer,
)
...
from extender.roberta_simulated import SimulatedRobertaForQuestionAnswering
...
def main():
    ...
    model = SimulatedRobertaForQuestionAnswering.from_pretrained(
        args.model_name_or_path,
        config=config,
    ...
    pprint(vars(args))
    num_centroids = args.bucket_size
    ...
    if args.attn_variant in ['extender', 'routing_trained', 'routing_extended']:
        ...
        try:
            if isinstance(centroids[0], torch.Tensor):
                num_centroids = centroids[0].shape[-2]
            else:
                num_centroids = centroids[0].nun_centroids
        except:
            num_centroids = args.bucket_size
        for i, layer in enumerate(model.base_model.encoder.layer):
    ...
    if args.do_train and (args.local_rank == -1 or torch.distributed.get_rank() == 0):
        ...
        # correct way to load with simulated roberta (not needed though)
        # model = SimulatedRobertaForQuestionAnswering.from_pretrained(
        #     args.output_dir,
        #     variant=args.attn_variant,
        #     projections_path=args.projections_path,
    ...
    if args.do_eval and args.local_rank in [-1, 0]:
        ...
        for checkpoint in checkpoints:
            ...
            output_eval_file = os.path.join(
                args.output_dir,
                "eval_results_squad1_{}_{}_{}_{}.txt".format(
                    args.attn_variant, args.finetune_part, num_centroids, args.window_size
                )
            )
            with open(output_eval_file, "w") as writer:
                logger.info("***** Eval results squad {} *****".format(args.attn_variant))
