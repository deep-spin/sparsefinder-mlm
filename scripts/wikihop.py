...
def preprocess_wikihop(infile, tokenizer_name='roberta-large', sentence_tokenize=False):
    ...
    def sent_tok(s):
        return tokenizer.tokenize(''.join(['<s> ' + sent + '</s>' for sent in sent_tokenize(normalize_string(s))]),
...
class WikihopQADataset(Dataset):
    ...
    def _convert_to_tensors(self, instance):
        ...
        return torch.tensor(candidate_ids), torch.tensor(support_ids), torch.tensor(predicted_indices), torch.tensor(
            [answer_index])
...
class WikihopQAModel(LightningModule):
    ...
    def forward(self, candidate_ids, support_ids, prediction_indices, correct_prediction_index,
                return_predicted_index=False):
        """
    ...
    def _get_loader(self, split, fname=None, tokenize=False):
        ...
        dataset = WikihopQADataset(fname, is_train, tokenize=tokenize, tokenizer_name=self.args.tokenizer_name,
                                   sentence_tokenize=self.args.sentence_tokenize)
    ...
    def add_model_specific_args(parser):
        ...
        parser.add_argument("--truncate-seq-len", type=int, default=None,
                            help="Only consider this many tokens of the input, default is to use all of the available context")
