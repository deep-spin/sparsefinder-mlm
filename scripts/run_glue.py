...
from extender.roberta_simulated import SimulatedRobertaForSequenceClassification
...
class ModelArguments:
    ...
    attn_variant: str = field(
        ...
        metadata={"help": "attention variant to use in test-time",
                  "choices": "regular, extender, bigbird, longformer, openai_sparse, reformer, routing, "
                             ...
                             "distance_cos, distance_l2, reformer_fixed"}
    )
    ...
    bucket_size: int = field(
        default=0,
        metadata={"help": "Num of buckets actually"}
    )
...
def main():
    ...
    model = SimulatedRobertaForSequenceClassification.from_pretrained(
        ...
        bucket_size=model_args.bucket_size,
    ...
    if model_args.attn_variant != 'regular':
        print('loading projections from {}'.format(model_args.projections_path))
    ...
    if model_args.attn_variant in ['extender', 'routing_trained', 'routing_extended']:
        print('loading centroids from {}'.format(model_args.centroids_path))
        # centroids = torch.load(model_args.centroids_path, map_location=lambda storage, loc: storage)
        centroids = get_layer_centroids(model_args.attn_variant, model_args.centroids_path, 12, 12)
        for i, layer in enumerate(model.base_model.encoder.layer):
    ...
    if training_args.do_eval and training_args.local_rank in [-1, 0]:
        ...
        for eval_dataset in eval_datasets:
            ...
            output_eval_file = os.path.join(
                ...
                f"eval_results_{eval_dataset.args.task_name}_{model_args.attn_variant}_{model_args.finetune_part}.txt"
            )
    ...
    #             result[key.replace('eval_', 'test_')] = result.pop(key)
    #
    #         layers = trainer.model.base_model.encoder.layer
    #         add_stats_to_result_dict(result, layers, metric='sparsities_gold', prefix="test")
    #         add_stats_to_result_dict(result, layers, metric='sparsities_pred', prefix="test")
    #         add_stats_to_result_dict(result, layers, metric='precisions', prefix="test")
    #         add_stats_to_result_dict(result, layers, metric='recalls', prefix="test")
    #         add_stats_to_result_dict(result, layers, metric='exacts', prefix="test")
    #         add_stats_to_result_dict(result, layers, metric='entropies_q', prefix="test")
    #         add_stats_to_result_dict(result, layers, metric='entropies_k', prefix="test")
    #
