...
"""
important: activate a virtualenv with newest version of hf's `transformers`
"""
import torch
    ...
    attn_variant: str = field(
        ...
        metadata={"help": "attention variant to use in test-time",
                  "choices": "regular, extender, bigbird, longformer, openai_sparse, reformer, routing, "
                             "routing_trained, routing_extended"}
    )
...
def main():
    ...
    # Load pretrained model and tokenizer manually (we know it is a Roberta model)
    #
    from transformers import RobertaTokenizerFast
    from extender.roberta_simulated_new import SimulatedRobertaForSequenceClassification
    tokenizer = RobertaTokenizerFast.from_pretrained(model_args.model_name_or_path)
    model = SimulatedRobertaForSequenceClassification.from_pretrained(
        model_args.model_name_or_path,
        config=config,
        variant=model_args.attn_variant,
        projections_path=model_args.projections_path,
        centroids_path=model_args.centroids_path,
        window_size=model_args.window_size,
        cls_size=model_args.cls_size,
        top_clusters=model_args.top_clusters,
    ...
    if model_args.attn_variant in ['extender', 'routing_trained', 'routing_extended']:
        projections = torch.load(model_args.projections_path, map_location=lambda storage, loc: storage)
    ...
    if model_args.finetune_part == 'head':
        # Freeze the encoder
        for param in model.base_model.parameters():
            param.requires_grad = False

    elif model_args.finetune_part == 'all_but_projections':
    ...
    if training_args.do_eval:
        ...
        for eval_dataset, task in zip(eval_datasets, tasks):
            ...
            add_stats_to_result_dict(metrics, trainer.model.base_model.encoder.layer, metric='sparsities_gold')
            add_stats_to_result_dict(metrics, trainer.model.base_model.encoder.layer, metric='sparsities_pred')
            add_stats_to_result_dict(metrics, trainer.model.base_model.encoder.layer, metric='precisions')
            add_stats_to_result_dict(metrics, trainer.model.base_model.encoder.layer, metric='recalls')
            add_stats_to_result_dict(metrics, trainer.model.base_model.encoder.layer, metric='exacts')
            add_stats_to_result_dict(metrics, trainer.model.base_model.encoder.layer, metric='entropies_q')
            add_stats_to_result_dict(metrics, trainer.model.base_model.encoder.layer, metric='entropies_k')
    ...
    if training_args.do_predict:
        ...
        # do not compute graph stats for prediction
        for i, layer in enumerate(trainer.model.base_model.encoder.layer):
            layer.attention.self.compute_graph_stats = False
...
def add_stats_to_result_dict(result, layers, metric='sparsities_gold', prefix="eval"):
    tensor_stats = get_average_results(get_list_of_metrics_per_layer(layers, metric='sparsities_gold'))
    result[prefix+"_"+metric+"_avg_all"] = str(tensor_stats.mean().item())
    result[prefix+"_"+metric+"_avg_per_layer"] = str(tensor_stats.mean(1).numpy())
    result[prefix+"_"+metric+"_layers_and_heads"] = str(tensor_stats.flatten().numpy())
